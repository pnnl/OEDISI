...
First Created on Tue Aug 9 2022 by Dylan Tarter
...
class nllsClass_v2:
    ...
    def run(self,t,x):
        ...
        if(x.size == x.shape[0]): # if its a 1D array...
            if(self._doROCOF):
                FPP = self.__estimateFreq1Phase_periodogram(t[ :-4],x[ :-4],self._fs)
                FP  = self.__estimateFreq1Phase_periodogram(t[1:-3],x[1:-3],self._fs)
                F   = self.__estimateFreq1Phase_periodogram(t[2:-2],x[2:-2],self._fs)
                FN  = self.__estimateFreq1Phase_periodogram(t[3:-1],x[3:-1],self._fs)
                FNN = self.__estimateFreq1Phase_periodogram(t[4:  ],x[4:  ],self._fs)
            ...
            """
            if(self._doROCOF):
                FPP = self.__estimateFreq_periodogram(t[ :-4],x[ :-4,:3],self._fs)
                FP  = self.__estimateFreq_periodogram(t[1:-3],x[1:-3,:3],self._fs)
                F   = self.__estimateFreq_periodogram(t[2:-2],x[2:-2,:3],self._fs)
                FN  = self.__estimateFreq_periodogram(t[3:-1],x[3:-1,:3],self._fs)
                FNN = self.__estimateFreq_periodogram(t[4:  ],x[4:  ,:3],self._fs)
            else:
    ...
    @staticmethod
    def __estimateFreq_periodogram(t,x,fs):
        f1, Pxx_den1 = signal.periodogram(x[:,0],fs)
        f0hat_1 = f1[np.argmax(Pxx_den1)]
        
        f1, Pxx_den2 = signal.periodogram(x[:,1],fs)
        f0hat_2 = f1[np.argmax(Pxx_den2)]
        
        f1, Pxx_den3 = signal.periodogram(x[:,2],fs)
        f0hat_3 = f1[np.argmax(Pxx_den3)]
        
        f0hat = (f0hat_1 + f0hat_2 + f0hat_3)/3;
    ...
    @staticmethod
    def __estimateFreq1Phase_periodogram(t,x,fs):
        f, Pxx_den = signal.periodogram(x,fs)
        f0hat = f[np.argmax(Pxx_den)]
        return f0hat
